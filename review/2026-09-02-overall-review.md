...
さらに改修版へのレビューを3周し、そこで出た指摘も同じブランチで解消した。
**5件中4件は「今回の修正が作った/残した穴」**で、修正そのものを対象にレビューを回さなければ
そのまま入っていた:
...
改修中のレビューで挙がったが、今回の範囲外として意図的に残した2件:
...
- **`cmd_close` だけ前提条件が `require_open` に載っていない。** インラインの状態確認が
  残っており、復旧手順も添えていない。今回 `promote` / `merge` に入れた形へ揃えるのが
  一貫するが、`close` は現状でも正しく弾けているため機能的な欠陥ではない。
