...
改修中のレビューで挙がったが、今回の範囲外として意図的に残した1件:

- **`check_file.sh` の WARN はフック経由ではエージェントに届かない。** `post_edit.sh` は
  ...
  lint スクリプトが無い構成ではどちらの経路でも報告されない(`.js`/`.mjs`/`.cjs` は
  `node --check` が構文だけは拾うが、`.ts`/`.tsx`/`.jsx` は何も拾わない)。
