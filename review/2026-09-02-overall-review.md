...
さらに改修版そのものを対象にレビューを7周回し、そこで出た指摘18件も同じブランチで解消した。
**その多くが「今回の修正が作った/残した穴」**で、修正を対象にレビューを回さなければ
そのまま入っていた。主なものを挙げる:

| 再レビューでの指摘 | 性質 |
|---|---|
| ESLint 致命的エラー時に構文検査へ倒さず、**一度も検査しないまま素通し**していた | ⑥の修正が作った穴(`node --check` が `elif` にあり到達しない) |
| `updated_status_text` が frontmatter 内でも部分文字列置換で、`--category` の値に `status: open` と書くと値が壊れる | ①の修正が残した同型の穴(境界は絞ったが「キー」で見ていなかった) |
| `cmd_merge` の状態ガードが「すでにこのルールの出典です」より先に出て、**破壊的な retire を案内**していた | ②の修正が作った穴(案内どおり操作すると統合先ルールごと消える) |
| 終端していない frontmatter で `status_changed` の挿入が行を連結する | ①の修正が作った穴(手書きエントリのみ) |
| JSONC しか無いプロジェクトで `json 構文` が未検証のまま `PASS` | ④と同型で、④の修正では塞がっていなかった既存の穴 |
| JSONC を除外した結果、ステージごと消えて `--list-checks` からも「検査していない」ことが見えなくなった | 直上の修正が作った穴(`record_skip` の契約違反)。さらに `RESULTS` が空でなくなり「スタック未検出」の案内まで消していた |
| 行頭アンカーが TOML の許すインデント付き見出し(`  [tool.mypy]`)を取りこぼす | ⑤の修正が作った穴。`python.sh` の全ゲートへ一律に `^[[:space:]]*` を適用して解消 |
| `cmd_close` だけ前提条件が `require_open` に載らず、文言も復旧手順もドリフトしていた | 本ブランチが足した設計メモに自ら反していた |
| 護欄テストの `check_ids` が値を標準出力で返しており、失敗時の `fail` が subshell ごと捨てられていた | 護欄が自力で落ちられない状態(このブランチが追加したテスト) |

護欄テスト自身にも同じ穴が2度見つかった(走査が1つの書き方しか拾わない・否定形のアサーションだけで
素通しする)。**「テストがある」を根拠にしない**という本プロジェクトの原則は、
今回追加したテストにもそのまま適用する必要があった。

あわせて、既存データの救済(0.1.12 より前の二重昇華で作られた重複)は `require_open` では
救えないため、`find_rule_by_source` のエラーへ該当行と復旧手順を載せた。
`promote` / `merge` の `open` 前提は3言語 README へ記載した。

### PR #17 に対するレビュー(2026-09-03)で出た指摘4件

改修版を PR #17 として上げたのち、さらに独立したコードレビューを1周かけた。
4件が出て、いずれも同一ブランチで解消した。**3件は「今回の修正が作った/残した穴」で、
うち2件は本レポートが自ら書いた設計原則に反していた。**

| 指摘 | 性質 | 対応 | 護欄 |
|---|---|---|---|
| ⑦[P1] `require_open` が `verb` を無視し、`close` / `merge` にも promote 向けの文言(+ `retire` の案内)を返していた | ②の修正が残した穴。案内どおり `retire` するとルール本体が消え、目的の操作は `status=retired` で依然失敗する — **案内に従うほど壊れて先へ進めない** | 復旧案内を `_promoted_recovery` で verb ごとに分岐。`merge` では `retire` を「やり直しの手段」として案内しない | `tests/test_feedback_log_retire.sh`(走査で verb の書き漏れも検出) |
| ⑧[P2] `check_file.sh` の WARN が `events.jsonl` に残らず、被覆ゼロが初回通過率の**上昇**として現れていた | 申し送りとして残していた件の、記録側の半分 | `emit` が `harness_log_warn ... post_edit` で記録。`check_sev` が severity と検査IDを1つの入口で束ね、ラベルの書き漏れを構造的に防ぐ | `tests/test_check_file_severity.sh` / `tests/test_events_log.sh` |
| ⑨[P2] `anything_detected` の材料が `RESULTS` で、`--list-checks` では常に false へ退化していた | 直上の「ステージごと消さない」修正が新たに踏んだ非対称。通常実行には出る secretlint の案内が一覧からだけ消える | 材料を `RECORDED_CHECKS`(`run_stage` / `record_skip` の入口で加算)へ変更し、モードに依らない判定に | `tests/test_check_config.sh` |
| ⑩[P3] 宣言ゲートの走査が `-q` / `-qE` / `-qF` の固定列挙で、`-qs` / `-q --` / `>/dev/null` を素通ししていた。加えて対象ディレクトリが消えると否定形アサーションが空振りで成立 | ⑤の護欄自身の穴。**「設定ファイル名を固定列挙しない」と同じ誤りを護欄側で繰り返していた** | フラグ列を任意個の `-...` として受ける形へ。走査が実ゲートに触れていることを先に固定する肯定形アサーションを追加 | `tests/test_declaration_gates.sh` |

⑧について、**本文がエージェントへ届かない件は依然として申し送りのまま**である(下記)。
今回直したのは「記録が消える」側だけで、これは分けて扱うのが正しい —
記録の欠落は指標を汚染するため独立に有害であり、本文の配送とは修正範囲も検証方法も違う。

### このブランチで直していないもの(次回への申し送り)

- **`check_file.sh` の WARN 本文はフック経由ではエージェントに届かない。** `post_edit.sh` は
  `check_file.sh` が exit 0 のとき出力を捨てるため、`severity: warn` の指摘も今回追加した
  「ESLint を実行できませんでした」も、PostToolUse からは見えない(直接実行時のみ見える)。
  これは今回の変更で生まれたものではなく、WARN 経路が元から持っていた性質である。
  当初この申し送りには「フック経路での気づきは Stop 側の `$PM run lint` が担う」と
  書いていたが、**再レビューでこれが誤りだと分かった**: Stop 側の `node-lint` は
  `npm_script_exists lint` がゲートのため、`lint` スクリプトを持たないプロジェクトでは
  Stop でも報告されない。つまり「ESLint の設定が壊れている」状態は、
  lint スクリプトが無い構成ではどちらの経路でも**本文が**報告されない
  (`.js`/`.mjs`/`.cjs` は `node --check` が構文だけは拾うが、`.ts`/`.tsx`/`.jsx` は何も拾わない)。
  直すなら PostToolUse の JSON 出力(`additionalContext`)が候補だが、Codex 側の
  互換性確認が要るため独立した変更として扱うべき。**次回レビューの最優先候補。**
  なお⑧により、この状態が**記録**としては `events.jsonl` に残るようになったため、
  `stats` / `report` から「どの検査が何回素通ししたか」は追えるようになっている。
