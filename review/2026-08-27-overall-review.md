...
> **履歴資料:** これは 2026-08-27 時点の `main` に対するレビュー記録です。指摘は対応済みで、本文中の件数・行数・テスト数はレビュー実施時点のスナップショットです(現在の値とは一致しません)。現在の仕様は[プロジェクト概要](../README.md)、[設定ガイド](../docs/configuration.ja.md)、[スクリプト仕様](../scripts/README.md)と実装を参照してください。
