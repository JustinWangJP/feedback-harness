...
assert_contains "$CI_BODY" "runs-on: windows-latest" "PowerShell配布経路をWindows runnerで検証する"
...
assert_contains "$CI_BODY" "tests/run_tests.ps1" "PowerShell回帰テストをCIで実行する"
assert_contains "$CI_BODY" "scripts/check.ps1" "PowerShell版ハーネス自身をCIで検査する"
