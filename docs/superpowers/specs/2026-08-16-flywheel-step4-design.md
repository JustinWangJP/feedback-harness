...
> **履歴資料:** この文書で設計した P1〜P3 は実装済みです。現在の利用方法は[プロジェクト概要](../../../README.md)と[スクリプト仕様](../../../scripts/README.md)を参照してください。
...
|---|---|---|
| P3 シグナル型 | L37〜47（4種類のシグナルと、シグナルから共有成果物への具体的な対応関係） | 4分類を frontmatter のデータにし、反映先と rules.md の構造を型で決める |
| P1 イベントログ+stats | L69〜73（初回通過率・イテレーション数。ダッシュボードは作らない） | フックの成否から「機能しているか」を示す数値をテキストで出す。再発候補の列挙を自動化する |
...
明示指定が常に最優先。原因は failure の詳細な振り分けに使うため、従来どおり `--detail` に残す。signal は原因を置き換えず、記事 L45 の「原因に応じて改善する共有成果物を決める」という考え方を維持する。
