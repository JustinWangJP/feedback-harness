...
| 2026-08-24 | 再レビュー指摘(1・2)の修正 | agents/feedback-curator.md / skills/feedback-loop / lib.sh / tests | curator だけ `feedback_log.py` 直接実行のまま残り Git Bash でキュレーション経路が全滅していたのを `feedback.sh` へ移行し、`.py` 直接実行を走査で禁止。`command -v` が export された shell function を解決するため `tests/assert.sh` の shim が本番の interpreter 解決を乗っ取り、Windows 経路の検証テストが shim を検査していた問題を、解決側を実ファイル限定にして修正(各々 変異を入れて落ちることを確認) |
