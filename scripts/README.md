...
| `promote` | `<entry-id>` `--rule "<one generalized rule>"` | **Adds a new rule** to `rules.md` and marks the entry `promoted`. Only runs when the entry is `open` — promoting twice creates two rules citing the same source, after which `merge` and `retire` can no longer run for that id |
| `merge` | `<entry-id>` `--into <existing-rule-source-id>` `[--rule "<updated-text>"]` | Adds provenance to an **existing rule** without creating a new line, then marks the entry `promoted`. Use for recurrence of the same principle. Like `promote`, only runs when the entry is `open` |
