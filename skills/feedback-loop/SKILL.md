...
1. `bash "${CLAUDE_PLUGIN_ROOT}/scripts/init.sh" <対象プロジェクトパス>` を実行する(導入先にはまだ `scripts/` が無い — それを作るのが init.sh である)
...
2. 各ルールを「**維持 / 文言強化 / 退役候補**」の3分類でレポートし、ユーザーに提示する
3. 退役はユーザーの裁定後にのみ実行する(rules.md の手編集はしない):

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/feedback_log.py" retire <出典id> --reason "<退役理由>"
```
